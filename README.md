...
- `backend/scripts/run.sh` extracts that tarball into `frontend/dist`, exports `FRONTEND_DIST_DIR` with the absolute path, and then starts `uvicorn app.main:app`, so the deployment only needs to install the Python dependencies (`pip install -r requirements.txt`) and can serve the static UI without running Node on the platform.
